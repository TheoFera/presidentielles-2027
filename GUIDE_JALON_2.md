...
1. **Densité et apparition.** Au départ : **25 Neutres**, aucun allié offert à aucun candidat. Paris, Périurbain, Campagne, Retraités et Quartiers riches en ont chacun 4 ; Banlieue en a 5, répartis 2 / 2 / 1. Chaque sous-biome possède un plafond de population d’origine compris entre **8 et 16 PNJ**. Le jeu calcule automatiquement la fréquence à partir de la durée de campagne, du nombre initial et du plafond. Avec les réglages actuels, les apparitions sont espacées d’environ **26 à 52 jours de jeu**, soit **65 à 130 secondes**. Tous les sous-biomes atteignent leur plafond à l’arrivée du premier tour.
...
| --- | --- |
| world_layout.json → chaque sous-zone | initial_neutral_count, max_npcs_by_origin, points sociaux |
| world_layout.json → neutral_population_growth | activation, date cible et variation des intervalles calculés |
...
| game_balance.json → time | 2,5 secondes par jour de jeu |
...
- Chaque sous-biome compte tous ses PNJ selon leur origine, quel que soit leur rôle ou leur position actuelle. Une fois `max_npcs_by_origin` atteint, aucune nouvelle apparition n’a lieu. Le plafond total du monde est actuellement de **201 PNJ**. Les retours de démobilisation conservent leur origine et restent donc compris dans cette limite.
- Les délais sont recalculés automatiquement si la durée de la partie, la population initiale, le plafond ou le nombre de points sociaux change. Ils ne représentent aucune donnée sociologique.
