...
async function fits(page, name) {
  ...
  const bad = await page.evaluate(() => [...document.querySelectorAll('#start-menu :is(h1,h2,p,button,input,.qr-image,.qr-card),dialog[open] :is(h2,video,p,button)')].filter(e => {
    const b = e.getBoundingClientRect(); return b.width && b.height && (b.top < -1 || b.left < -1 || b.bottom > innerHeight + 1 || b.right > innerWidth + 1 || ['BUTTON', 'INPUT', 'ARTICLE', 'DIV'].includes(e.tagName) && (e.scrollHeight > e.clientHeight + 2 || e.scrollWidth > e.clientWidth + 2));
...
try {
  ...
  await host.locator('#create-room').click(); await host.locator('#invite-player').click();
  ...
  for (const [page, slot] of [[guest, 2], [third, 3]]) {
    ...
    for (const viewport of [{ width: 320, height: 568 }, { width: 667, height: 375 }, { width: 390, height: 844 }]) { await page.setViewportSize(viewport); await fits(page, `Réponse joueur ${slot} ${viewport.width}x${viewport.height}`); }
  }
  ...
  for (const [page, slot] of [[third, 3], [guest, 2]]) {
    ...
    await transfer(page.locator('.qr-image'), host, () => page.locator('#room-players').isVisible());
  ...
  await host.locator('#qr-back-lobby').click();
  ...
  for (const [p, faction] of [[host, 'philippe'], [guest, 'melenchon'], [third, 'le_pen']]) {
    await p.locator(`[data-choose="${faction}"]`).click();
    await p.locator(`[data-choose="${faction}"]`).filter({ hasText: 'Votre candidat' }).waitFor();
    await fits(p, 'Choix du candidat après connexion');
  }
  await host.locator('#launch-room:not([disabled])').click();
