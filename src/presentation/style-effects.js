  ...
  for (const power of state.powers) {
    if (power.fire_zone && power.fire_zone.expires_tick > state.tick) {
      ...
      for (let i = 0; i < 11; i++) {
        ...
        ctx.fillStyle = i % 2 ? '#ffd55e' : '#ff853a'; ctx.beginPath(); ctx.moveTo(fx - 6, m.groundY); ctx.quadraticCurveTo(fx - 10, feetY - h * .5, fx + 3, feetY - h); ctx.quadraticCurveTo(fx + 10, feetY - h * .4, fx + 6, m.groundY); ctx.fill();
      }
    }
    for (const [id, burn] of Object.entries(power.burns || {})) {
      ...
      if (target) { ctx.font = 'bold 18px sans-serif'; ctx.fillStyle = '#ffc365'; ctx.fillText('♨', renderer.screenX(target.x) - 8, m.groundY - m.characterHeight * (.4 + (target.combat?.height || 0))); }
    }
  }
  for (const c of state.candidates) {
    if (c.is_ko || c.disappeared) continue;
    const x = renderer.screenX(c.x), h = m.characterHeight, feetY = m.groundY - (c.combat?.height || 0) * h;
    if (c.ultimate_effect?.kind === 'EUROPE' && c.ultimate_effect.expires_tick > state.tick) {
      ...
      ctx.beginPath();
      for (let i = 0; i <= 40; i++) { const a = i / 40 * Math.PI * 2, r = 1 + .12 * Math.sin(i * 3 + time * 15); const px = x + Math.cos(a) * h * .48 * r, py = feetY - h * .55 + Math.sin(a) * h * .65 * r; if (i) ctx.lineTo(px, py); else ctx.moveTo(px, py); }
      ...
      for (let i = 0; i < 7; i++) { const a = i / 7 * Math.PI * 2 + time; ctx.fillText('★', x + Math.cos(a) * h * .58, feetY - h * .55 + Math.sin(a) * h * .7); }
      ctx.restore();
    }
    if (c.ultimate_effect?.kind === 'SCARF' && c.ultimate_effect.expires_tick > state.tick) {
      ...
      if (attack && attack.elapsed_ticks >= attack.windup_ticks && attack.elapsed_ticks < attack.windup_ticks + attack.active_ticks) {
        ...
        for (const [i, color] of ['#245ccb', '#fff7e9', '#e94959'].entries()) { ctx.strokeStyle = color; ctx.lineWidth = 4; ctx.beginPath(); ctx.moveTo(x, feetY - h * .6 + i * 4); ctx.bezierCurveTo(x + c.facing * reach * .4, feetY - h * .9, x + c.facing * reach, feetY - h * .3, x + c.facing * reach, feetY - h * .6 + i * 4); ctx.stroke(); }
      }
    }
    if (c.bardella_form) {
      const age = (state.tick - c.bardella_transition_tick) / hz;
      if (age < .4) { ctx.fillStyle = `rgba(205,231,255,${(1 - age / .4) * .8})`; ctx.beginPath(); ctx.ellipse(x, feetY - h * .5, h * .5, h * .7, 0, 0, Math.PI * 2); ctx.fill(); }
      ctx.fillStyle = '#eff6ff'; ctx.font = 'bold 11px sans-serif'; ctx.textAlign = 'center'; ctx.fillText('Bardella', x, feetY - h - 12);
    }
  }
  ctx.restore();
}
