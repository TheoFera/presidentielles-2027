...
import { wrap } from '../simulation/world.js';
...
export class CampaignDisplay {
 constructor(config){this.config=config;this.cards=new Map();this.seen=new Map();this.seed=null;this.lastTick=0;this.root=document.createElement('div');this.root.id='campaign-events';this.root.setAttribute('aria-live','polite');document.body.append(this.root);}
 update(state){
 ...
 for(const e of state.campaign_events){if(!this.seen.has(e.id))this.seen.set(e.id,e.status==='ACTIVE'||e.start_tick>=previousTick?now:-Infinity);const age=(now-this.seen.get(e.id))/1000,visible=state.phase==='CAMPAIGN'&&(e.status==='ACTIVE'||age<2.4);
 ...
 card.replaceChildren(heading,narrative,detail,timer);card.classList.toggle('arriving',e.category==='INSTANT'||age<2);card.classList.toggle('instant',e.category==='INSTANT');
 }
 }
}
export function drawCampaignScenery(renderer,state){
 ...
 const h=48+(z.index*17+i*11)%35,y=m.groundY-5;
