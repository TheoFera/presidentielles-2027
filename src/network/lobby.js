...
export function chooseCandidate(room, playerId, faction) {
  if (room.phase !== 'lobby' || room.players.length < 2) throw new Error('Connectez au moins deux joueurs avant de choisir les candidats.');
...
export const candidatesReady = room => room.players.length >= 2 && room.players.every(p => factions.includes(p.faction));
