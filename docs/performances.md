...
Vérifications sous Node 24 : **234 tests réussis**, export réussi. `scripts/validate-sprite-loading-browser.mjs` retarde volontairement une façade lointaine, vérifie que le démarrage reste bloqué, simule une erreur puis une nouvelle tentative réussie. Le parcours des 18 zones dans les deux sens ne présente aucune image manquante, éviction ou requête d’image supplémentaire. Le rapport est dans `artifacts/sprite-loading/browser-report.json`. Le test de rendu compare également **48 scènes identiques pixel par pixel**, avec les animations en cours de modification présentes dans les deux versions.

Pour reproduire, lancer le serveur sur le port 2037, définir `CAMPAIGN_TEST_NODE_MODULES` vers Playwright, puis `node scripts/validate-sprite-loading-browser.mjs`. Le chargement initial peut durer davantage et conserve plus de décors en mémoire. Sur la carte testée avec les tenues et pouvoirs actuels, les 148 images représentent environ 371 Mio de pixels RGBA décodés, hors copies GPU et surfaces de préparation : ce n’est pas une mesure de mémoire totale du navigateur. La consommation sur téléphone physique reste à contrôler ; aucune résolution d’image n’a été réduite.
